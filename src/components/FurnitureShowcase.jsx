...
const FurnitureShowcase = ({
  ...
  // Scroll-based category switching with image slide-up animation
  useEffect(() => {
    const section = sectionRef.current
    if (!section) return

    const ctx = gsap.context(() => {
      // Create a single ScrollTrigger that handles all layers
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          // Calculate which category transition we're in
          const currentTransition = scrollProgress * (totalCategories - 1)
          const currentIndex = Math.floor(currentTransition)
          const transitionProgress = currentTransition - currentIndex

          // Update active category based on scroll position
          const targetCategoryIndex = Math.min(Math.round(currentTransition), totalCategories - 1)
          const targetCategory = categories[targetCategoryIndex]
          if (targetCategory && activeCategory !== targetCategory.id) {
            setActiveCategory(targetCategory.id)
          }

          // Animate each layer
          categories.forEach((category, index) => {
            const layer = backgroundLayersRef.current[index]
            if (!layer || index === 0) return // Skip first layer, it's always visible

            if (index === currentIndex + 1) {
              // This is the layer currently transitioning in
              const clipValue = 100 - (transitionProgress * 100)
              gsap.set(layer, {
                clipPath: `inset(${clipValue}% 0 0 0)`,
              })
            } else if (index <= currentIndex) {
              // These layers are fully visible
              gsap.set(layer, {
  ...
  // Get active category's color theme
  const activeCategoryData = categories.find(cat => cat.id === activeCategory) || categories[0]
  const { primaryBgColor, secondaryBgColor, textColor } = activeCategoryData
  ...
  return (
    ...
    >
      ...
      {/* Background layers - each category gets its own layer */}
      <div className="background-layers">
        {categories.map((category, index) => (
          <div
            ...
            style={{
              backgroundImage: `url('${category.backgroundImage || backgroundImage}')`,
              clipPath: index === 0 ? 'none' : 'inset(100% 0 0 0)', // First layer fully visible, others hidden initially
              zIndex: index, // Stack layers in order
            }}
          />
        ))}
      </div>
